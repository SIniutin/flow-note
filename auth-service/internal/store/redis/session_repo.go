...
func (r *redisRepoImpl) CreateRefresh(ctx context.Context, s d.RefreshSession) error {
	if uuid.UUID(s.ID) == uuid.Nil || uuid.UUID(s.UserID) == uuid.Nil || len(s.TokenHash) == 0 {
	...
	pipe := r.rdb.Pipeline()
	pipe.Set(ctx, r.hashKey(s.TokenHash), s.ID.String(), ttl)
...
func (r *redisRepoImpl) GetRefresh(ctx context.Context, tokenHash []byte) (d.RefreshSession, error) {
	...
	if err := sessionID.UnmarshalText([]byte(sid)); err != nil {
		return d.RefreshSession{}, fmt.Errorf("parse session id: %w", err)
	}

	val, err := r.rdb.Get(ctx, r.fmtKey(sessionID)).Bytes()
	if err != nil {
		if errors.Is(err, redis.Nil) {
...
func (r *redisRepoImpl) RevokeRefresh(ctx context.Context, tokenHash []byte) error {
	...
	pipe.Del(ctx, r.fmtKey(sessionID))
