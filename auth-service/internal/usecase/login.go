...
func (c *LoginUser) Exec(ctx context.Context, l *d.UserLoginRequest) (d.User, d.TokenPair, error) {
	...
	switch {
	case l.Email != "":
		email, convErr := d.NewEmail(l.Email)
		if convErr != nil {
			return d.User{}, d.TokenPair{}, convErr
		}
		u, err = c.u.GetByEmail(ctx, *email)
	case l.Login != "":
		login, convErr := d.NewLogin(l.Login)
		if convErr != nil {
			return d.User{}, d.TokenPair{}, convErr
		}
		u, err = c.u.GetByLogin(ctx, *login)
	default:
		return d.User{}, d.TokenPair{}, d.ErrValidation
	}
	if err != nil {
	...
	accessToken, accessExp, err := c.i.NewAccess(u.ID.String())
	if err != nil {
		return d.User{}, d.TokenPair{}, err
	}
