...
func (f fakeIssuer) NewAccess(userID string) (string, time.Time, error) {
	if f.accessErr != nil {
...
func TestLoginUserExecSuccess(t *testing.T) {
	...
	user, err := userRepo.Create(context.Background(), d.User{
...
func TestLoginUserExecInvalidPassword(t *testing.T) {
	...
	_, err = userRepo.Create(context.Background(), d.User{
		ID:    d.UserID(uuid.New()),
		Email: d.Email("user@example.com"),
		Login: d.Login("user"),
	}, d.PasswordHash{
